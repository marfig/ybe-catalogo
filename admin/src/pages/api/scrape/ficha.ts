...
import { skuDeOrigen } from '../../../lib/scrape/origen.ts';
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    /**
     * Las fotos de la galería son del COLOR DE ESTA FICHA, no del modelo: las
     * miniaturas de los hermanos ya se descartaron en la extracción, porque colgarlas
     * acá le pondría a la variante la foto del color equivocado.
     */
    const sku = ficha.colorOrigen ? skuDeOrigen(ficha.codigo, ficha.colorOrigen) : null;
    ...
    return json({
      ...
      // `null` si el origen no dio el color: las fotos quedan sin variante a la que ir.
      sku,
      fotos: sku ? ficha.fotos : [],
