  ...
  for (const url of ficha.fotos ?? []) {
    try {
      ...
      if (!tipo.startsWith('image/')) {
        ...
        if (cuerpo.error) anotarProblema(`Foto de ${ficha.codigo ?? sku}`, cuerpo.error);
        continue;
      }
      ...
      /**
       * EL HASH TIENE QUE COINCIDIR. El Worker lo calculó sobre los mismos bytes y con
       * el mismo algoritmo, así que si difiere es que el cuerpo llegó cortado. Sin este
       * corte la foto se guardaría bajo una clave que el Worker nunca vio: el dedupe se
       * rompe y R2 junta duplicados, en silencio y para siempre.
       */
      if (delWorker && subida.hash16 !== delWorker) {
        anotarProblema(
          `Foto de ${ficha.codigo ?? sku}`,
      ...
      if (vinculo.error) anotarProblema(`Foto de ${ficha.codigo ?? sku}`, vinculo.error);
    } catch (error) {
      anotarProblema(
        `Foto de ${ficha.codigo ?? sku}`,
